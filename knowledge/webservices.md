...
- `global.is_tokenized`, `has_password`, `is_secured` tell you whether a plain player URL will
  play; see `knowledge/security.md`,
...
- `html5_sources[].html5_source.manifest` points at the media's file manifest — a JSON index of
  every encoded file, not an `.m3u8`. `knowledge/player.md` explains how to go from there to a
  playable URL.
...
- **it is returned everywhere a media is, and `playlist` filters on it from webservices 5.25.**
  ...
  neither takes the parameter. The API filters on it too, from 5.48 (`knowledge/api.md`).
...
subtitle tracks. Requires a whitelisted IP, so it is a backend-only call. `knowledge/player.md`
